...
# end
    ...
    # scores =  exp.(scores .- scores_logsumexp)
        ...
        # for j in 1:new_trace_choices[:init => :init_state => :N]#could be more efficient to do these loops at once 
        #     si = new_trace_choices[:init => :init_state => :init_objs => j => :sprite_index]
        #     if si > sprite_index
        #         new_trace_choices[:init => :init_state => :init_objs => j => :sprite_index] = si - 1  
        #     end
        # end
        ...
        # for noti in 1:tr[:init => :init_state => :N]
        #     i = tr[:init => :init_state => :N] - noti + 1 #going in reverse order to not mess up the cascades
        ...
        #         new_trace_choices[:init => :init_state => :init_objs => i => :sprite_index] = si - 1 #doesn't mix well with shiftallobjs 
        #     end
...
#         sprite_index = tr[:init => :init_state => :init_objs => obin => :sprite_index]

#         #for now, new second sprite index and new obj index arent perf 
#         new_second_sprite_index ~ uniform_discrete(tr[:init => :num_sprites], tr[:init => :num_sprites])
#         #new_second_sprite_index ~ uniform_discrete(1, tr[:init => :num_sprites])
...
#         backward_choices[:obin2] = tr[:init => :init_state => :N] + 1 #fix this 
...
#         for obin in 1:tr[:init => :init_state => :N]
#             #is object sprite index is the sprite index
#             if tr[:init => :init_state => :init_objs => obin => :sprite_index] == sprite_index
#                 pos = tr[:init => :init_state => :init_objs => obin => :pos]
...
#                 new_trace_choices[:init => :init_state => :init_objs => newN => :pos] = Vec(newy, newx)
#                 new_trace_choices[:init => :init_state => :init_objs => newN => :sprite_index] = new_second_sprite_index
#             end 
#         end 

#         new_trace_choices[:init => :init_state => :N] = newN

#     else
#         #merge 
#         obin1, obin2 = split_merge[:obin1], split_merge[:obin2]
#         sprite_index1, sprite_index2 = tr[:init => :init_state => :init_objs => obin1 => :sprite_index], tr[:init => :init_state => :init_objs => obin2 => :sprite_index]

#         newsprite, relativey1, relativex1, relativey2, relativex2 = get_merged_sprite(tr[:init => :init_state => :init_objs => obin1], tr[:init => :init_sprites => sprite_index1], tr[:init => :init_state => :init_objs => obin2], tr[:init => :init_sprites => sprite_index2])
...
#         for obin in 1:N
#             # if sprite index 1
#             if tr[:init => :init_state => :init_objs => obin => :sprite_index] == sprite_index1
#                 pos = tr[:init => :init_state => :init_objs => obin => :pos]
#                 newy, newx = pos.y + relativey1, pos.x + relativex1
#                 new_trace_choices[(:init => :init_state => :init_objs => obin => :pos)] = Vec(newy, newx)
#             end 
#             #if sprite index 2
#             if tr[:init => :init_state => :init_objs => obin => :sprite_index] == sprite_index2
#                 pos = tr[:init => :init_state => :init_objs => obin => :pos]
#                 newy, newx = pos.y + relativey2, pos.x + relativex2
#                 new_trace_choices[(:init => :init_state => :init_objs => obin => :pos)] = Vec(newy, newx)
#                 new_trace_choices[(:init => :init_state => :init_objs => obin => :sprite_index)] = sprite_index1
#             end
#         end 
...
#         #not perfect 
#         new_second_sprite_index ~ uniform_discrete(sprite_index+1, num_sprites + 1)#is this sound? 
...
#         sprite_index1 ~ uniform_discrete(1, num_sprites)
#         sprite_index2 ~ uniform_discrete(sprite_index1+1, num_sprites) #this isn't correct
#     end 
# end
...
#                 newN += 1
#                 newy, newx= pos.y + yshift, pos.x + xshift
#                 tr[:init => :init_state => :init_objs => newN => new_second_sprite_index => :pos] = Vec(newy, newx)
#                 tr[:init => :init_state => :init_objs => newN => new_second_sprite_index => :sprite_index] = new_second_sprite_index
#             end 
#         end 

#         tr[:init => :init_state => :N] = newN
...
#     #mask things 
#     maski ~ uniform_discrete(1, tr[:init => :num_sprites])	
...
#     #color things 
#     colori ~ uniform_discrete(1, tr[:init => :num_sprites])
...
#     hilist ~ all_rand_hi_wi(1:tr[:init => :num_sprites], [tr for _ in 1:tr[:init => :num_sprites]])
#     @show hilist
#     wilist ~ all_rand_hi_wi(1:tr[:init => :num_sprites], [tr for _ in 1:tr[:init => :num_sprites]])
#     @show wilist
# end 
...
#     for i in range (1, tr[:init => :num_sprites])
#         mask = tr[:init => :init_sprites => i => :mask]
#         height, width = size(mask)
#         hi ~ uniform_discrete(1, height)	#don;t actually want hi in the trace, look at split nerge example. one func for randomness
#         wi ~ uniform_discrete(1, width)
...
#     #update num sprites 
#     tr, = mh(tr, select(:num_sprites))

#     #update object positions
#     for i=1:tr[:init => :init_state => :N]#init defined in model
#         tr, = mh(tr, select((:init => :init_state => :init_objs => i => :pos))) #correct? 
#     end
...
#         tr, = mh(tr, select((:init => :init_state => :init_objs => i => :sprite_index))) 
#     end
