...
@gen function sm_helper(tr, s1, s2, relpos1i, relpos1j, relpos2i, relpos2j)
    ...
    objs_with_sprite_s1 = []
    for objin in 1:N
        if tr[:init => :init_state => :init_objs => objin => :sprite_index] == s1
    ...
    s1_obj_pos = tr[:init => :init_state => :init_objs => s1_obj => :pos]
...
@gen function get_split_merge(tr)
    ...
    if({:split_or_merge} ~ bernoulli(num_sprites == 1 ? 1 : 0.5)) 
        #splitting a sprite 

        s1 ~ uniform_discrete(1, num_sprites)
        #s2 is any sprite except s1 
        s2 ~ uniform_not_x(s1, 1, num_sprites)
