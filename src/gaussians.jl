...
function rand_gauss(H, W, K)
    ...
    scale_y = (rand() * 0.5 - 0.5 / 2 + 1.0) * scale_x
    vcat(Float32[
            rand()*H,
            rand()*W,
            scale_x,
...
function test_gradients(; target=nothing, lr=300, lr_min=50, lr_decay=0.99, N=100, iters=20, check=false, mode=:reverse, device=:gpu, log_every=1)
    ...
    mode === :forward && grad_step_forward_mode(canvas_dual, target, gaussians, transmittances, 0.0f0, cfg)
    mode === :reverse && grad_step_reverse_mode(canvas, target, gaussians, transmittances, dgaussians, 0.0f0)
...
@inline function normalize_gaussians_inner(gaussians, G)
    ...
    # clamp opacity
    gaussians[G_OPACITY, G] = clamp(gaussians[G_OPACITY, G], 0.0f0, 1.0f0)
    ...
    gaussians[G_R, G] = clamp(gaussians[G_R, G], 0.0f0, 1.0f0)
    gaussians[G_G, G] = clamp(gaussians[G_G, G], 0.0f0, 1.0f0)
    gaussians[G_B, G] = clamp(gaussians[G_B, G], 0.0f0, 1.0f0)
    return nothing
end
...
@inline function render_pixel(py, px, gaussians, N)
    ...
    for G in 1:N
        ...
        clamped_density = min(density, 0.99f0)
...
function draw_kernel_inner_backward(canvas, gaussians, transmittances, target, dgaussians, cx, cy, ymin, ymax, xmin, xmax, N) # ,::Val{N}) where N
    ...
    dloss_dr = (r_loss_signed >= 0.0f0 ? -1.0f0 : 1.0f0) / (H * W)
    dloss_dg = (g_loss_signed >= 0.0f0 ? -1.0f0 : 1.0f0) / (H * W)
    dloss_db = (b_loss_signed >= 0.0f0 ? -1.0f0 : 1.0f0) / (H * W)
    ...
    for G in reverse(1:N)
        ...
        dclamped_density_ddensity = density < 0.99f0 ? 1.0f0 : 0.0f0
