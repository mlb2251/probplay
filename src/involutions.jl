...
function flatmatrixindex_from_position(pos, H, W)
    ...
    return floor(Int,pos.x-1)*H + floor(Int,pos.y)
end
...
function shiftallobjs(min_obj, max_obj, shift_by, new_trace_choices, tr)
    ...
    for i in min_obj:max_obj
        new_trace_choices[:init => :init_state => :init_objs => i+shift_by => :pos] = tr[:init => :init_state => :init_objs => i => :pos]
        new_trace_choices[:init => :init_state => :init_objs => i+shift_by => :sprite_index] = tr[:init => :init_state => :init_objs => i => :sprite_index]
    end 
end
...
if add samples sprite id, sprite mask, sprite color, list of obj ids, list of positions"""
    num_sprites = length(env_of_trace(tr).sprites)
    if({:add_or_remove} ~ bernoulli(num_sprites == 1 ? 1 : 0.5)) 
        #adding 
        ...
        #removing 
        rm_sprite_index ~ uniform_discrete(1, num_sprites)
    end 
end 
        ...
        if in_mask 
            ...
            for dydx in neighbor_list
                ...
                if pos.y + dy > 0 && pos.y + dy <= H && pos.x + dx > 0 && pos.x + dx <= W
                    neighbor_pos = Vec(pos.y + dy, pos.x + dx)
...
@gen function dd_add_remove_sprite_random(tr, heatmap)
    ...
    if remove samples sprite id
    if add samples sprite id, sprite mask, sprite color, list of obj ids, list of positions"""
    num_sprites = tr[:init => :num_sprites]
    if({:add_or_remove} ~ bernoulli(num_sprites == 1 ? 1 : 0.5)) 
        #adding 
...
function add_remove_sprite_involution(tr, add_remove_random, forward_retval, proposal_args)
    ...
    if add_remove_random[:add_or_remove]
        ...
        snp1num = M + 1
        new_trace_choices[:init => :num_sprites] = snp1num
        ...
        #add the new objects 
        new_trace_choices[:init => :init_state => :N] = N + add_remove_random[:num_objs]
        for i in 1:add_remove_random[:num_objs]
            new_trace_choices[:init => :init_state => :init_objs => N + i => :pos] = add_remove_random[:positions => i => :pos]
            new_trace_choices[:init => :init_state => :init_objs => N + i => :sprite_index] = M + 1
        end

        backward_choices[:add_or_remove] = false
        backward_choices[:rm_sprite_index] = M + 1

    else 
        #remove 
        sprite_index = add_remove_random[:rm_sprite_index]
        new_trace_choices[:init => :num_sprites] = M - 1

        positions = []
        #remove each object
        #remember to change N 
        # newN = tr[:init => :init_state => :N]
        Nremoved = 0
        ...
        for i in 1:N
            si = tr[:init => :init_state => :init_objs => i => :sprite_index]
            ...
            if si == sprite_index
                ...
                if si > sprite_index 
                    new_trace_choices[:init => :init_state => :init_objs => i - Nremoved => :sprite_index] = si - 1
                end 
                #push!(change_plan, -Nremoved) #change this
            end
        end

        new_trace_choices[:init => :init_state => :N] = N - Nremoved

        
        #remove the sprite
        if sprite_index != M #idk if this is causing the error/neccesary
            #if not the last sprite, shift all the sprites 
            shiftallsprites(sprite_index+1, M, -1, new_trace_choices, tr)
        end
...
function dd_add_remove_sprite_involution(tr, add_remove_random, forward_retval, proposal_args)
    ...
    if add_remove_random[:add_or_remove]
        ...
        #add the new objects 
        new_trace_choices[:init => :init_state => :N] = tr[:init => :init_state => :N] + add_remove_random[:num_objs]
        for i in 1:add_remove_random[:num_objs]
            new_trace_choices[:init => :init_state => :init_objs => tr[:init => :init_state => :N] + i => :pos] = add_remove_random[:positions => i => :pos]
            new_trace_choices[:init => :init_state => :init_objs => tr[:init => :init_state => :N] + i => :sprite_index] = tr[:init => :num_sprites] + 1
        end

        backward_choices[:add_or_remove] = false
        backward_choices[:rm_sprite_index] = tr[:init => :num_sprites] + 1

    else 
        #remove 
        sprite_index = add_remove_random[:rm_sprite_index]
        new_trace_choices[:init => :num_sprites] = tr[:init => :num_sprites] - 1
        ...
        old_sindicies = [] 
        change_plan = [] #1 means removed, negative/zero means how much to go down
        for i in 1:tr[:init => :init_state => :N] 
            si = tr[:init => :init_state => :init_objs => i => :sprite_index]
            #push!(old_sindicies, si)
            if si == sprite_index
                #push!(change_plan, 1)
                pos = tr[:init => :init_state => :init_objs => i => :pos]
        ...
        new_trace_choices[:init => :init_state => :N] = tr[:init => :init_state => :N] - Nremoved

        
        #remove the sprite
        if sprite_index != tr[:init => :num_sprites] #idk if this is causing the error/neccesary
            #if not the last sprite, shift all the sprites 
            shiftallsprites(sprite_index+1, tr[:init => :num_sprites], -1, new_trace_choices, tr)
        end
...
@gen function dd_get_random_new_color(tr, i)
    ...
    for objind in 1:tr[:init => :init_state => :N]
        if tr[:init => :init_state => :init_objs => objind => :sprite_index] == i
            pos = tr[:init => :init_state => :init_objs => objind => :pos]
            


            #ACTUALLY GOTTA ONLY CHECK STUFF IN BOUNDS 
            starti, startj, stopi, stopj = get_bounds(tr[:init => :init_state => :init_objs => objind], tr[:init => :init_sprites => i], 1, H, 1, W)
...
@gen function dd_get_drift(tr, i, heatmap)
    ...
    #get old x, y
    oldx = tr[:init => :init_state => :init_objs => i => :pos].x
    oldy = tr[:init => :init_state => :init_objs => i => :pos].y
...
function shift_involution(tr, drift, forward_retval, proposal_args) 
    ...
    newpos = Vec(newy, newx)
    new_trace_choices[(:init => :init_state => :init_objs => i => :pos)] = newpos
...
function add_remove_involution(tr, add_remove_stuff, forward_retval, proposal_args)
    ...
    if add_remove_stuff[:add_or_remove]
        ...
        new_trace_choices[(:init => :init_state => :init_objs => ind_to_add => :pos)] = add_remove_stuff[:pos]
        new_trace_choices[(:init => :init_state => :init_objs => ind_to_add => :sprite_index)] = add_remove_stuff[:sprite_index]
        backward_choices[:remove_obj_index] = ind_to_add
        new_trace_choices[(:init => :init_state => :N)] = N + 1
        ...
        # #FOR DEBUGGING 
        # spriteind = tr[:init => :init_state => :init_objs => remove_obj_index => :sprite_index]
        # mask = tr[:init => :init_sprites => spriteind => :mask]
        # render_matrix(mask)
       
        pos = tr[:init => :init_state => :init_objs => remove_obj_index => :pos]
        backward_choices[:pos] = pos
        backward_choices[:sprite_index] = tr[:init => :init_state => :init_objs => remove_obj_index => :sprite_index]
...
function layer_involution(tr, layer_swap, forward_retval, proposal_args)
    ...
    new_trace_choices[(:init => :init_state => :init_objs => l1 => :sprite_index)] = tr[:init => :init_state => :init_objs => l2 => :sprite_index]
    new_trace_choices[(:init => :init_state => :init_objs => l2 => :sprite_index)] = tr[:init => :init_state => :init_objs => l1 => :sprite_index]
    new_trace_choices[(:init => :init_state => :init_objs => l1 => :pos)] = tr[:init => :init_state => :init_objs => l2 => :pos]	
    new_trace_choices[(:init => :init_state => :init_objs => l2 => :pos)] = tr[:init => :init_state => :init_objs => l1 => :pos]
